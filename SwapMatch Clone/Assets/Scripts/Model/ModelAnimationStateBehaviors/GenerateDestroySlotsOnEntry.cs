...
{
    public class GenerateDestroySlotsOnEntry : StateMachineBehaviour
    {
        private static readonly int DestroySlotsGenerated = Animator.StringToHash("DestroySlotsGenerated");
        ...
        public override void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
        {
            ...
            GenerateDestroySlots(objsTracker, varsTracker, startingPosition);

            animator.SetBool(DestroySlotsGenerated, true);
        }
        
        private void GenerateDestroySlots(ViewObjsTracker objsTracker, KeyVarsTracker varsTracker, Vector2 startingPosition)
        {
            objsTracker.destroySlotObjs = new GameObject[varsTracker.gridSize.y,varsTracker.gridSize.x];
            for (var row = 0; row < varsTracker.gridSize.y; row++)
            {
                for (var column = 0; column < varsTracker.gridSize.x; column++)
                {
                    objsTracker.destroySlotObjs[row, column] = new GameObject("DestroySlot" + row + "-" + column);
                    objsTracker.destroySlotObjs[row, column].transform.SetParent(objsTracker.destroyGridObj.transform);
                    objsTracker.destroySlotObjs[row, column].transform.localPosition =
                        CalculateGemSlotPosition(varsTracker, startingPosition, new Vector2Int(column, row));
                    objsTracker.destroySlotObjs[row, column].transform.localScale = Vector3.one;
                }
            }
        }
