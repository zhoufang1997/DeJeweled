...
namespace Model.ModelAnimationStateBehaviors
{
    public class GenerateDestroySlotsOnEntry : StateMachineBehaviour
    {
        ...
        {
            objsTracker.killSlotObjs = new GameObject[varsTracker.gridSize.y,varsTracker.gridSize.x];
            for (var row = 0; row < varsTracker.gridSize.y; row++)
            {
                for (var column = 0; column < varsTracker.gridSize.x; column++)
                {
                    objsTracker.killSlotObjs[row, column] = new GameObject("DestroySlot" + row + "-" + column);
                    objsTracker.killSlotObjs[row, column].transform.SetParent(objsTracker.killGridObj.transform);
                    objsTracker.killSlotObjs[row, column].transform.localPosition =
                        CalculateGemSlotPosition(varsTracker, startingPosition, new Vector2Int(column, row));
                    objsTracker.killSlotObjs[row, column].transform.localScale = Vector3.one;
                }
            }
        }
